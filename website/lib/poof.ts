...
type Puff = { x: number; y: number; size: number; born: number };
 ...
 *  animation happened. */
export function poof(x: number, y: number, size = POOF_SIZE_PX): number {
...
export function poofElement(
  el: HTMLElement,
  {
    anchor = 'center',
    hide = true,
  }: { anchor?: 'center' | 'top'; hide?: boolean } = {},
): number {
