...
test('the header nav has the Programs and language drop-downs', () => {
  for (const page of Object.values(pages)) {
    const html = read(page);
    ...
    assert.match(programs, /href="#leaders"/);
    assert.doesNotMatch(nav, /<a href="#fellowship">[^<]+<\/a>\s*<a/, `${page} still lists Fellowship as a top-level link`);
    const list = /id="lang-menu-list"[\s\S]*?<\/ul>/.exec(html)?.[0] ?? '';
    assert.match(list, /class="nav-menu-list" hidden>/, `${page} dropdown list is not hidden at rest`);
